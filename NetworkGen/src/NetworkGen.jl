    ...
    while connected == false
        ...
        for i in 1:n_nodes
            if i != smallest
                ...
                for j in 1:n_nodes
                    if r_range[1] <= get_prop(mg,i,:η) <= r_range[2]
                        add_edge!(mg,i,j)
                    end
                end
            end
...
function plot_graph(g)
    return graphplot(g, self_edge_size = 0.2, layout = Shell(), curve_distance = 0.1, node_size = 20)
end
