...
@everywhere function system_evolve(func::ODEFunction{true}, init_vals::Matrix{Float64}, p::ModelParams, time_closest::Matrix{Int64},time_integrate::Matrix{Int64}, time_inc::Float64, target::Vector{Float64}, invader::Int64, max_iters::Int64)
    ...
    for iters in 1:max_iters
        ...
        expr = JuMP.@expression(model,
            ...
            (target[invader] .- (init_sol.u[1][invader] .+ (Mt*dx)[invader]))^2
        ...
        if (sol_integrate[end][invader] < -20) && (all(x -> x >= -20, sol_integrate[end][Not(invader)]))#any(success_vec)
            return sol_integrate.u, perturb, term_status, tc_array, points, "Success", num_err, times_thru
        end
    end
    #println("max iters")
    return sol_integrate.u, perturb, term_status, tc_array, points, "MaxIters", num_err, times_thru
end
...
@everywhere function perturb_dynamics(dir, max_iters, succ_save_dir, fail_save_dir, allee_type::Union{Type{Absent},Type{Hill},Type{Strong}})
    ...
    for file in ProgressBar(files)
        ...
        @sync @distributed for ind in eachindex(nodes)
            ...
            if end_state == "Success"
                jldsave(string("$succ_save_dir/",replace(file,string(dir,"/")=>"")[1:end-5],"_$node",".jld2"),best_sol = best_sol, perturb = perturb, term_status = term_status, tc_array = tc_array, init_cons = init_cons, end_state = end_state)
            else
                jldsave(string("$fail_save_dir/",replace(file,string(dir,"/")=>"")[1:end-5],"_$node",".jld2"),best_sol = best_sol, perturb = perturb, term_status = term_status, tc_array = tc_array, init_cons = init_cons, end_state = end_state)
            end
        end
    end
